...
;; Extension example
(proto:define-schema opacity-color-wheel
    (:package proto_test
     :optimize :speed)
  (proto:define-message opacity-color-wheel
      (:conc-name color-wheel-)
    (proto:define-enum opacity-color-status ()
      deprecated
      unready)
    (name   :type string)
    (colors :type (proto:list-of opacity-color) :default ())
    (status :type opacity-color-status :default :unready))
  (proto:define-message opacity-color
      (:conc-name color-)
  ...
  (proto:define-extend opacity-color ()
    ((opacity 1000) :type (or null integer)))
  (proto:define-message get-color-request
      (:conc-name color-request-)
    (wheel :type opacity-color-wheel)
    (name  :type string))
  (proto:define-message add-color-request
      (:conc-name color-request-)
    (wheel :type opacity-color-wheel)
    (color :type opacity-color))
  (proto:define-service color-wheel ()
    (get-opacity-color (get-color-request opacity-color)
      :options (:deadline 1.0))
    (add-opacity-color (add-color-request opacity-color)
      :options (:deadline 1.0))))

(qtest:define-test extension-serialization ()
  (let* ((wheel  (make-instance 'opacity-color-wheel :name "Colors"))
         (color1 (make-instance 'opacity-color :r-value 100 :g-value 0 :b-value 100))
         (rqst1  (make-instance 'add-color-request :wheel wheel :color color1))
         (color2 (make-instance 'opacity-color :r-value 100 :g-value 0 :b-value 100))
      ...
      (qtest:assert-true (string= (with-output-to-string (s)
                                  ...
                                  (with-output-to-string (s)
                                    (proto:print-text-format
                                      (proto:deserialize-object 'add-color-request ser1)  nil :stream s)))))
    (let ((ser2 (proto:serialize-object-to-stream rqst2 'add-color-request :stream nil)))
      (qtest:assert-true (string= (with-output-to-string (s)
                                    (proto:print-text-format rqst2 nil :stream s))
                                  (with-output-to-string (s)
                                    (proto:print-text-format
                                      (proto:deserialize-object 'add-color-request ser2) nil :stream s)))))
